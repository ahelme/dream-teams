...
|------|---------|---------|
| `new-team.sh` | 1.1.0 | Stand up a fully-wired `comfyume-v1` clone for a new team. |
...
| `file-paths-registry.sh` | 1.0.0 | The single place hard-coded Mello paths live. Edit here when repos move. |
...
```jsonc
{
  ...
  "identity": "spl-team"           // 6. teams-chat.local.md → teams-chat/<slug>.md ("" = none)
}
```
...
.claude/agent_tools/apply-links.sh --dir /path/to/comfyume-v1
```
...
```
Clones the app repo, branches off `all-teams-testing` (default), creates the always-on symlinks,
seeds `settings.local.json`, creates the team's identity file, writes an all-on `team-links.json`,
then calls `apply-links.sh` to wire the shared symlinks + identity. Ends with a verify pass
(clean tree, all symlinks resolve, `0` `.gitignore` identity-negations). Refuses to clobber an
existing clone.
...
launched with `--add-dir /home/dev/projects/claude-code-comfyume-teams`** — that flag is what
actually pulls the Teams-repo `CLAUDE.md` into context. The `CLAUDE_CODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD=1`
...
The robust mechanism is a **per-team launch alias** in `~/.zshrc` (one direct + one tmux), mirroring
the existing `claude-adm`/`claude-cui`/… set:

```bash
# direct launch
alias claude-<slug>='cd <clone-path> && CLAUDE_CODE_ADDITIONAL_DIRECTORIES_CLAUDE_MD=1 \
  claude --add-dir /home/dev/projects/claude-code-comfyume-teams -c --dangerously-skip-permissions'
...
provisioned clone whose operator types plain `claude` will silently run without the Teams `CLAUDE.md`.
This actually happened to `spl-team` (2026-07-08): the clone lived at a non-standard path
(`comfyume-new/team-clones/spl-team/…`), no `claude-spl` alias existed, and the session ran context-blind
until the alias was added. **When provisioning a team, add both aliases and start a new shell.** See
wiki **[[Claude Code Team Launchers]]**. (TODO: fold alias creation into `new-team.sh`.)
...
| Tool | Purpose | Repo |
|---|---|---|
| **graphify-kit** | Drop-in [graphify](https://pypi.org/project/graphifyy/) knowledge-graph setup for any git repo: scoping (`.graphifyignore`), a versioned Claude Code nudge hook, and **ignition** (initial build + auto-updating post-commit/post-checkout git hooks). Self-contained — no Teams-repo/symlink dependency. | https://github.com/ahelme/graphify-kit |
...
For a **comfyume-v1 clone** its unique job is **ignition** (`.graphifyignore`, the nudge hook, and
the `graphify-out/` gitignore already ship via committed files + Teams symlinks). Run it *after*
the Teams symlinks so the nudge step no-ops on the identical hook. Related: #592, #597.

## Guard rails (do not break)

- Never `!`-negate `settings.local.json` or `teams-chat.local.md` in a `.gitignore` — that
  re-arms the April 2026 identity-overwrite incident.
- Never `cp -r` a clone (dereferences the shared symlinks). Use `new-team.sh`.
- Never edit another team's identity file.

## See also

- ADR: `agent_docs/ADR/2026-07-05-teams-clone-config-and-identity.md` (identity + gitignore model)
- ADR: `agent_docs/ADR/2026-07-05-ops-team-provisioning.md` (why script over template-copy)
- Wiki: **Team Setup and Configuration** (operator guide)
